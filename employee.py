...
class Employee:
    def __init__(self,root):
        ...
        Main_frame.place(x=10,y=120,width=1500,height=560)
        ...
        upper_frame.place(x=10,y=10,width=1490,height=270)
        ...
        lbl_doj.grid(row=2,column=2,sticky=W,padx=2,pady=7)
        ...
        # ID_Proof
        lbl_id = Label(upper_frame,font=('arial',11,'bold'),text='ID:',bg='white')
        lbl_id.grid(row=4,column=0,sticky=W,padx=2,pady=7)

        txt_proof = ttk.Entry(upper_frame,textvariable=self.var_idproof,width=22,font=('arial',11,'bold'),state= "disabled")
        ...
        btn_ShowAll = Button(search_frame,text='Show All',font=('Arial',11,'bold'),width=14,bg='blue',fg='white',command=self.fetch_data)
        ...
        self.employee_table = ttk.Treeview(table_frame,columns=('dep','name','degi','email','address','dob','doj','idproof','gender','phone','country','salary',),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        emp_details = ['dep','name','degi','email','address','dob','doj','idproof','gender','phone','country','salary']
        ...
        emp_details_value = ['Department','Name','Degignition','Email','Address','DOB','DOJ','id','Genter','Phone','country','Salary']
        ...
        self.employee_table.heading('idproof',text='ID')
    ...
    def add_data(self):
        if self.var_dep.get()=='' or self.var_email.get()=='':
            ...
            try:
                conn = mysql.connector.connect(host='localhost',port=3306,user='root',password='Shre_u@1213',database='mydata')
                my_cursor = conn.cursor()
                my_cursor.execute('insert into employeel(Department,Name,Designition,Email,Address,DOB,DOJ,Gender,Phone,Country,Salary) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(
                self.var_dep.get(),
                self.var_name.get(),
                ...
                query = 'insert into employeel (Department,Name,Designition,Email,Address,DOB,DOJ,Gender,Phone,Country,Salary) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)' % (
                    self.var_dep.get(),
                    self.var_name.get(),
                    self.var_designition.get(),
                    self.var_email.get(),
    ...
    def fetch_data(self):        
        conn = mysql.connector.connect(host='localhost',port=3306,user='root',password='Shre_u@1213',database='mydata')
        my_cursor = conn.cursor()
        my_cursor.execute('select * from  employeel')
        ...
        if self.var_dep.get()=='' or self.var_email.get()=='':
            ...
            try:
                ...
                if update>0:
                    conn = mysql.connector.connect(host='localhost',port=3306,user='root',password='Shre_u@1213',database='mydata')
                    my_cursor = conn.cursor()
                    my_cursor.execute('update employeel set Department=%s,Name=%s,Designition=%s,Email=%s,Address=%s,DOB=%s,DOJ=%s,Gender=%s,Phone=%s,Country=%s,Salary=%s where id=%s',(
self.var_dep.get(),
self.var_name.get(),
self.var_designition.get(),
self.var_email.get(),
        ...
        if self.var_idproof.get()=="":
            ...
            try:
                ...
                if Delete>0:
                    conn = mysql.connector.connect(host='localhost',port=3306,user='root',password='Shre_u@1213',database='mydata')
                    my_cursor = conn.cursor()
                    sql = 'delete from employeel where id=%s'
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo('Delete','Employee Successfuly Deleted',parent=self.root)
            except Exception as es:
                messagebox.showerror('Error',f'Due to:{str(es)}',parent=self.root)
    ...
    def search_data(self):
        if self.var_com_search.get()=='' or self.var_search == '':
            ...
            try:
                conn = mysql.connector.connect(host='localhost',port=3306,user='root',password='Shre_u@1213',database='mydata')
                my_cursor = conn.cursor()
                my_cursor.execute('select * from employeel where ' +str(self.var_com_search.get())+" LIKE '%s"+str(self.var_search.get()+"%'"))
